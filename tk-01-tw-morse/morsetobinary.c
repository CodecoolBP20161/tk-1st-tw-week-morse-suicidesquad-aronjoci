...
int MorseToBinary(char input[], char output[]) {
	...
	for(int i=0; input[i] != '\0'; i++){
		...
		switch(input[i]){
			case '.':
				*(output + index_of_byte) = *(output + index_of_byte) | (1<<(shift_correction));
				number_of_valid_bits += 1;
				break;

			case '-':
				if(shift_correction > 1){
					*(output + index_of_byte) =  *(output + index_of_byte) |  7<<(shift_correction - 2);
				}
				else if(shift_correction == 1){
					*(output + index_of_byte) =  *(output + index_of_byte) |  3<<(shift_correction - 1);
